...
for protectedAreaPair in protectedAreaPairs:
    ...
    # Calc Impermeability as Corridor divided by LCP Length
    impTempRaster = "%scratchWorkspace%\\" + "imp" + paIDA + "-" + paIDB
    gp.SingleOutputMapAlgebra_sa(corridorTempRaster + " / " + str(lcpLength), impTempRaster)
    ...
    # Extract Impermeability cells only within Corridor Envelope
    imeTempRaster = "%scratchWorkspace%\\" + "ime" + paIDA + "-" + paIDB
    gp.ExtractByMask_sa(impTempRaster, envTempRaster, imeTempRaster)
...
for pair in pairList:
    ...
    gp.SingleOutputMapAlgebra_sa("(" + str(maxOverallImpermeability) + " - " + imeTempRaster + ") / " + str(impermeabilityDifference), prxTempRaster)
    ...
    if deleteTemps == "true":
        gp.Delete_management(imeTempRaster)
